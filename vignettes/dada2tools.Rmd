...
Theis_merged1 <- convert_merged(merged = Theis_merged1, char_range = 1:8, num_range = 9:ncol(Theis_merged1))
```
...
Theis_ASV <- Theis_merged1 %>% select_if(., is.numeric) %>% select_if(colnames(.) %in% Theis_META_filt$Run)
Theis_TAX <- Theis_merged1[, 1:8]
...
Before barplotting, data must be agglomerated at the genus level since barplots will be comparative across studies. This is done with `agglomerate()` on the normalized dataset.
...
Theis_preheatbar <- heatmap_prep(df = Theis_agg_P, df_tax = Theis_agg_tax_P, class_col = "Genus", df_meta = Theis_META_P, select_ASVs = Theis5, mean_ab_cutoff = FALSE)
colnames(Theis_preheatbar) <- sub(pattern = "(.*)(-)(.*)", replacement = "\\1", x = colnames(Theis_preheatbar))
colnames(Theis_preheatbar)[match("Methylobacterium-Methylorubrum-Methylobacterium", colnames(Theis_preheatbar))] <- "Methylobacterium-Methylorubrum"
```
...
It is quite easy to transition to barplotting from heatmapping, so it is recommended to run `heatmap_prep()` on a data set before `barplot_prep()`. 

```{r}
Theis_prebar <- barplot_prep(Theis_preheatbar, df_meta = Theis_META_P, type_col = "Del_GA", study = "Theis", DECONTAM_dataset = FALSE)
Theis_prebar
```

### Plotting Barplots {#Barplot}

Finally the we can make the barplots. Note, we do not need to store these plots to be able to plot them. 

```{r fig.width= 8, fig.height=8}
Theis_barplot <- barplotting(df = Theis_prebar, study = "Theis", legend_cols = 2, leg_title = "Genera")
Theis_barplot
```
...
Now that we have a data set run through DECONTAM, we need to re-plot everything. We can take the `Theis_true_taxa` data set and use the same process to generate PCoA plots, heatmaps, and barplots. 

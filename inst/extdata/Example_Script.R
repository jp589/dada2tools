...
library(RPushbullet) #I use this to notify me when runs are completed. #optional
...
#Eliminate samples (put them in a separate directory) with quality profiles which dip below 30 before 200 "cycles".
#Then re-run Block 1. Quality profile plotting can be omitted with plot_quality_profiles(plot = FALSE).
...
#trim sequences around target length and adjust if necessary. If there is a wide range of merged sequence lengths it would be good to trim them. This is currently set for V1-2 anticipated length.
#if silencing this line then change everything below that is seqtab2 to seqtab.
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% 252:287]
...
#Proceed to install package dada2tools and follow along for post-analysis.

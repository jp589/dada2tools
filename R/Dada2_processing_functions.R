...
blaster <- function(seqs, names, alignments = 1) {
  ...
  system(paste0("blastn -query ", paste0(here::here(), "/blaster.fasta -num_alignments "), alignments, " -num_threads 3 -out ", paste0(here::here(), "/Blasted.tsv -outfmt \"6 qseqid evalue bitscore staxids pident qcovs stitle\" -db "), paste0(system.file("extdata", package="dada2tools"), "/16S_ribosomal_RNA")))  results <- utils::read.table(file = "Blasted.tsv", sep = "\t", stringsAsFactors = FALSE)
